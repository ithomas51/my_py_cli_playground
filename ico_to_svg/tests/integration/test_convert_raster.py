...
class TestRasterConversion:
    ...
    def test_convert_with_background(self, multi_size_ico: Path, tmp_path: Path) -> None:
        ...
        convert_ico_to_svg(
            str(multi_size_ico), str(output), mode="raster", background="#ffffff"
        )
        assert output.exists()
        content = output.read_text()
        assert "data:image/png;base64," in content
    ...
    def test_all_available_sizes(
        self, multi_size_ico: Path, tmp_path: Path, size: str
    ) -> None:
    ...
    def test_convert_without_size_uses_largest(
    ...
    def test_convert_single_size_ico(
    ...
    def test_convert_with_pathlib_paths(
        self, multi_size_ico: Path, tmp_path: Path
    ) -> None:
    ...
    def test_transparent_background_explicit(
        self, multi_size_ico: Path, tmp_path: Path
    ) -> None:
